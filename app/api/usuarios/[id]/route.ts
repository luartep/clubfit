...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const {
    nombre, email, telefono, foto, fotoDescriptor, planTipo,
    planInicio, planVencimiento, activo, huellaId, huellaCredencial,
  } = body;

  // Las fechas de ingreso/término se editan tal cual las envía el admin
  // desde el formulario (ya sean las calculadas automáticamente o modificadas a mano).
  const rows = await sql`
    UPDATE usuarios SET
      ...
      plan_inicio = COALESCE(${planInicio || null}, plan_inicio),
      plan_vencimiento = COALESCE(${planVencimiento || null}, plan_vencimiento),
